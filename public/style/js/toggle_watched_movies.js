function checkIfWatched(user_id, movie_id){
    ...
    try {
      if (window.StatusStore){
        $(`[id='add_watched_movie_${movie_id}']`).hide();
        $(`[id='remove_watched_movie_${movie_id}']`).hide();
        StatusStore.getOne('movie', String(movie_id)).then(function(st){
          if (st && st.w===true) $(`[id='remove_watched_movie_${movie_id}']`).show();
          else $(`[id='add_watched_movie_${movie_id}']`).show();
        });
        return;
      }
    } catch(_){}
...
async function movieAddWatched(user_id, movie_id, movie_runtime, user_key) {
    ...
    try {
        ...
        if (data && (data.status === 'ok' || data.status === 200)){
            $(`[id='add_watched_movie_${movie_id}']`).hide();
            $(`[id='remove_watched_movie_${movie_id}']`).show();
        ...
        $(`[id='add_watched_movie_${movie_id}']`).show();
        $(`[id='remove_watched_movie_${movie_id}']`).hide();
